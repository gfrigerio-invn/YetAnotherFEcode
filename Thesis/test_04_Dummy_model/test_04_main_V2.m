...
%% --- 2. SETUP DIRECTORY ---
test_name = sprintf('Shock_%dg_%fs_%dxspring_Ode15_V2', impulse_g, tmax);
...
if n_dof_per_node < 2
    error('Il modello non ha abbastanza DOF per applicare un impulso in 2D.');
end

% Versore di direzione dell'impulso nel piano XY (norma unitaria per nodo)
impulse_dir = impulse_sign * [cosd(impulse_angle_deg); sind(impulse_angle_deg)];

dir_vector = zeros(n_dofs_fom, 1);
dir_vector(1:n_dof_per_node:n_dofs_fom) = impulse_dir(1);   % DOF X
dir_vector(2:n_dof_per_node:n_dofs_fom) = impulse_dir(2);   % DOF Y
...
% --- Energia di riferimento per il controllo d'errore ---
        ...
        for phi = array_linModes
            ...
            if run_ROM_MT
                ...
                [t_rom, q_rom_mt] = solverMT.solve(tmax, dt, q0_mt, qd0_mt, F_mt_handle, ...
                    ...
                    'ContactPenalty', k_contact, 'ModelType', 'MC',...
                    'ProjectionMatrix', Pc_mt,'Eref', Eref, 'RelTol', bench_RelTol, 'OutputTimes', t_common);
                cpu_time = toc; 
            ...
            if run_ROM_MC
                ...
                [t_rom, q_rom_mc] = solverMC.solve(tmax, dt, q0_mc, qd0_mc, F_mc_handle, ...
                    'ContactTargetDOF', contact_dofs, 'ContactGap', gaps_array, ...
                    'ContactPenalty', k_contact, 'ModelType', 'MC',...
                    'ProjectionMatrix', Pc_mc, 'Eref', Eref, 'RelTol', bench_RelTol, 'OutputTimes', t_common);
                cpu_time = toc; 
            ...
            for r_idx = 1:3
                if run_flags(r_idx)
                    ...
                    Pc_r = rom_obj.Pc;
                    if strcmp(rom_types{r_idx}, 'Rubin')
                        [gaps_run, k_run] = rom_obj.contact_params(gaps_array, k_contact);
                    else
                        gaps_run = gaps_array;  k_run = k_contact;   
                    end
                    offline_time = toc(tic_offline);
                    ...
                    [t_rom, q_rom] = solverROM.solve(tmax, dt, q0_r, qd0_r, F_r_handle, ...
                        'ContactTargetDOF', rom_contact_dofs, 'ContactGap', gaps_run, ...
                        'ContactPenalty', k_run, 'ModelType', rom_types{r_idx},...
                        'Eref', Eref, 'RelTol', bench_RelTol, 'OutputTimes', t_common); 
                    cpu_time = toc; 
